...
function DemoPageContent({ pathname }) {
  return (
    ...
    >
      ...
      {pathname === "/profile" && <ForSharksPage />}
      {pathname === "/meeting" && <MeetingPage />}
      {pathname === "/forsharks" && <SharkDash />}
      {pathname !== "/dashboard" &&
        pathname !== "/profile" &&
        pathname !== "/meeting" &&
        pathname !== "/forsharks" && (
  ...
  return (
    <Box sx={{ display: "flex" }}>
      ...
      >
        ...
        >
          Hello, <span style={{ color: "#00e676" }}>SHARKS!</span>
        </Typography>
...
function SharkDash() {
  ...
  React.useEffect(() => {
    ...
    const fetchSharksData = async () => {
      try {
        const response = await fetch("http://localhost:8080/api/sharks/all");
        const data = await response.json(); // Assuming the response is in JSON format
        setSharks(data); // Set the data into state
      } catch (error) {
        console.error("Error fetching data:", error);
      }
    };

    fetchSharksData(); // Call the function to fetch data
  ...
  return (
    <Box
      sx={{
        background: "linear-gradient(to bottom, #00251a, #004d40)",
        color: "white",
        minHeight: "100vh",
        textAlign: "center",
        padding: "50px 20px",
    ...
    >
      ...
      <Typography
        ...
        sx={{ fontWeight: "bold", textTransform: "uppercase", mb: 3 }}
      >
        Explore <span style={{ color: "#00e676" }}>Startups</span> You Like
      </Typography>
      ...
      <Box sx={{ mb: 3 }}>
        ...
        >
          {showLikedOnly ? "Show All Startups" : "Show Liked Startups"}
        </Button>
        <Grid container spacing={3} justifyContent="center">
          {sharks.map((sharks) => (
            <Grid item key={sharks.id}>
              <Card sx={{ maxWidth: 300, backgroundColor: "#004d40" }}>
                <CardMedia
                  component="img"
                  height="140"
                  image={sharks.image || "../src/assets/images/Logo.png"}
                  alt={sharks.name}
                />
                <CardContent>
                  <Typography variant="h6">{sharks.name}</Typography>
                  <Typography variant="subtitle2" sx={{ mb: 1 }}>
                    {sharks.title}
                  </Typography>
                  <Typography variant="body2">{sharks.bio}</Typography>
                  <Button
                    variant="contained"
                    sx={{
                      backgroundColor: likedCards.includes(sharks.id)
                        ? "#00e676"
                        : "#f4511e",
                      mt: 2,
                    }}
                    onClick={() => handleLikeClick(sharks.id)}
                  >
                    {likedCards.includes(sharks.id) ? "Liked" : "❤ Like"}
                  </Button>
                </CardContent>
              </Card>
            </Grid>
